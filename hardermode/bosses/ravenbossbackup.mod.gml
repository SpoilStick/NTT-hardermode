...
//}
//{ RavenBoss
//majority of code by saniblues, mutilated code (and sprites) by blaac
...
if instance_number(enemy) < (enemies/2) && instance_exists(Player){
	...
	var f = instance_nearest(p.x+random_range(-160,160),p.y+random_range(-160,160),Floor)
	if point_distance(f.x,f.y,p.x,p.y)<32{
		f = instance_nearest(p.x+random_range(-160,160),p.y+random_range(-160,160),Floor)
		}
	else{
	...
	if fork(){
		...
		if mod_variable_exists("mod","options","intropan") and mod_variable_get("mod","options","intropan") == "1"{
			if UberCont.opt_bossintros == 1{
				mod_script_call("mod", "bossintros", "boss_trigger","RAVEN COMMANDER",global.ravenintrobg,global.ravenintro,global.ravenintrofg,self,60);
				}else
				mod_script_call("mod", "bossintros", "boss_trigger", "", mskNone,mskNone,mskNone,self,0)
			wait 90{ alrm1 = 1; alrm0 = 1}
			}else
			if UberCont.opt_bossintros == 1{
				mod_script_call("mod", "bossintros", "boss_trigger","RAVEN COMMANDER",global.ravenintrobg,global.ravenintro,global.ravenintrofg,instance_nearest(x,y,Player),0);
				wait 90{ alrm1 = 1; alrm0 = 1}
				}
			}
		}
...
#define ravenboss_callbackup
	repeat(argument0*(GameCont.loops+1)){
...
#define ravenboss_step
			...
			if (distance_to_object(target) > range_max * 1.25 && angry && !--angry) || random(12) < 1{
				...
				if !collision_line(x,y,target.x,target.y,Wall,0,1){
					if distance_to_object(target) <= range_max && distance_to_object(target) > range_min{
						if random(5) < 3{
							walk = 20
							direction = (gunangle + choose(60,-60) + random_range(-10,10)) * -1;
							alrm1 = 1;
							}
						else{
							walk = 20 + irandom(30);
							direction = gunangle + choose(60,-60) + random_range(-10,10);
							alrm0 = walk + irandom(10);
							}
						}
					//else{
					//	alrm3 = 1;// Slam the motherfucker
					//	}
					}
	...
	///shoot missile
	if (alrm1 && !--alrm1) and flying = false{
		if instance_exists(target){
			gunangle = point_direction(x,y,target.x,target.y);
	...
	if sprite_index = spr_chrg{
		hitid = [spr_chrg,"RAVEN COMMANDER"];
	...
	if (image_index + image_speed >= image_number){
		if sprite_index = spr_idle || sprite_index = spr_walk || sprite_index = spr_hurt || sprite_index = spr_land{
			if speed > friction
				...
				sprite_index = spr_idle;
			hitid = [spr_idle,"RAVEN COMMANDER"];
			meleedamage = 0;
			}
		else
	...
	/// Makes it so that, when angry, the boss will always try to squash the player
    if instance_exists(target) && angry = 0{
        flight_x = target.x;
        flight_y = target.y;
		}
...
#define ravenboss_draw
	...
	if gunangle >= 180 && z = 0 draw_sprite_ext(sprGatlingBazooka,0,x-lengthdir_x(wkick,gunangle),y-z-lengthdir_y(wkick,gunangle),1,right,gunangle,c_white,1);
#define ravenrocket_step
	image_angle = direction
	if speed<14{
		motion_add(direction,0.1)
		if instance_exists(target) and "object_index" in target and instance_exists(creator)
		motion_add(point_direction(x,y,target.x,target.y),0.1)
		}
	with instance_create(x,y,Smoke)
		motion_add(other.direction+180,random(2))
	if random(5)<1
	with instance_create(x,y,TrapFire){
		motion_add(other.direction,random(3))
		team = other.team
		}
#define ravenrocket_explode
...
if instance_exists(Player)
and mod_variable_exists("mod","options","bossoutros") and mod_variable_get("mod","options","bossoutros") == "1"
mod_script_call("mod", "bossintros", "boss_trigger", "", mskNone,mskNone,mskNone,self,0)
#define boltstickfly
if instance_exists(creator){
	x=creator.x
	y=creator.y-creator.z
